...
export function LogoCloud() {
  return (
    <section className="py-8 md:py-16 px-6 relative z-20">
      <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true, margin: "-20px" }} className="max-w-5xl mx-auto text-center" style={{ contain: 'content' }}>
        ...
        <div className="logo-cloud-grid flex flex-wrap items-center justify-center gap-10 md:gap-16">
          {LOGO_PROVIDERS.map((p, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-20px" }} transition={{ delay: i * 0.08 }}>
              <motion.div animate={{ y: [0, -12, 0] }} transition={{ duration: 3 + (i % 3), repeat: Infinity, ease: 'easeInOut', delay: i * 0.2 }} className="opacity-40 hover:opacity-90 transition-opacity duration-500 grayscale hover:grayscale-0 will-change-transform">
  ...
  return (
    <section className="section-content py-12 md:py-32 px-6 max-w-[1100px] mx-auto relative z-20">
      ...
      <div className="hiw-grid flex md:grid md:grid-cols-3 gap-6 md:gap-8 relative overflow-x-auto snap-x snap-mandatory [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] pb-8 -mx-6 px-6 md:mx-0 md:px-0">
        ...
        {steps.map((s, i) => (
          <motion.div key={i} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-50px" }} transition={{ delay: i * 0.15, duration: 0.7 }} className="text-center relative min-w-[280px] md:min-w-0 snap-center flex-shrink-0" style={{ contain: 'content' }}>
            <motion.div animate={{ y: [0, -12, 0], scale: [1, 1.05, 1], rotate: [0, i % 2 ? 4 : -4, 0] }} transition={{ duration: 3.5, repeat: Infinity, ease: 'easeInOut', delay: i * 0.3 }} className="w-12 h-12 md:w-16 md:h-16 mx-auto mb-3 md:mb-6 rounded-[12px] md:rounded-2xl glass-panel border-white/10 flex items-center justify-center text-lg md:text-2xl relative z-10 bg-omin-black will-change-transform">{s.icon}</motion.div>
...
export function ModelLibrary() {
  ...
  return (
    <section id="models" className="section-content py-12 md:py-32 px-6 max-w-[1400px] mx-auto relative z-20">
      ...
      <div className="model-grid grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 overflow-hidden" style={{ contain: 'content' }}>
        {visible.map((m, i) => (
          <motion.div key={m.name} initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, margin: "-20px" }} transition={{ delay: (i % 8) * 0.05 }} className="glass-panel p-4 md:p-6 rounded-[1rem] md:rounded-[2rem] relative overflow-hidden group hover:border-white/20 transition-all cursor-default will-change-transform">
            <motion.div animate={{ y: [0, -5, 0] }} transition={{ duration: 3 + (i % 2), repeat: Infinity, ease: 'easeInOut', delay: (i % 5) * 0.15 }} className="mb-2 md:mb-4 scale-75 md:scale-100 origin-left will-change-transform"><BrandLogo slug={m.slug} color={m.color} name={m.name} size={28} /></motion.div>
